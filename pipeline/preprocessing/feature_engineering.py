...
class FeatureEngineer:
    ...
    def __init__(self,target_column: str = TARGET_COLUMN,date_column: str = DATE_COLUMN):
    ...
    def create_rolling_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in windows:
            ...
            df[f"rolling_mean_{window}"] = (df[self.target_column].shift(1).rolling(window).mean())

        std_windows = [4, 12]

        for window in std_windows:

            df[f"rolling_std_{window}"] = (df[self.target_column].shift(1).rolling(window).std())
